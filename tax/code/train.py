...
print('target_char_indices', target_char_indices)
print('target_indices_char', target_indices_char)
...
def vectorize_fold(fold1, fold1_t, fold1_t2, fold1_t3, fold1_t4, divisor, divisor2):
    ...
    for line, line_t, line_t2, line_t3, line_t4 in zip(lines, lines_t, lines_t2, lines_t3, lines_t4):
        for i in range(0, len(line), step):
            ...
            if i == 0:
                continue
            # this would be prefix of length 1
            if i == 1:
                continue            
...
if args.train:
    ...
    with open(os.path.join("results", eventlog_name + "_training_time.txt"), "w") as file:
...
if args.test:
    ...
    with open("results/" + eventlog_name +"_next_event.log", "w") as file:
        ...
        precision = precision_score(y_true, y_a_pred, average="weighted", zero_division=0)
        recall = recall_score(y_true, y_a_pred, average="weighted", zero_division=0)
        f1 = f1_score(y_true, y_a_pred, average="weighted", zero_division=0)
